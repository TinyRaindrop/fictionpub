...
class LinkResolver:
    ...
    def resolve(self) -> None:
        ...
        for entry in other_links:
            ...
            link_type = (
                LinkType.REGULAR
                if (entry.link_type == LinkType.INVALID and target_doc)
                else entry.link_type
            )
    ...
    def _determine_link_type(
        self, a: etree._Element, target_doc: FileInfo | None
    ) -> LinkType:
        """
        ...
        if (
            ...
            and "backlink" not in a.get("class", "")
        ):
    ...
    def _apply_link(self, entry: LinkEntry) -> None:
        ...
        match link_type:
            ...
            case _:
                log.warning(
                    f"Unexpected LinkType for id='{a.get('id')}' with href='{a.get('href', '')}'"
                )
                return
    ...
    def _apply_noteref(
        self, a: etree._Element, target_id: str, extra_class: str = ""
    ) -> None:
    ...
    def _mark_broken(self, a: etree._Element, target_id: str) -> None:
        ...
        if "backlink" in a.attrib.get("class", ""):
            log.warning(
                f"Note id='{target_id.rstrip('-ref')}' exists, but is never refenced."
            )
        else:
            log.warning(
                f"Broken internal link: target id='{target_id}' not found in any document."
            )
