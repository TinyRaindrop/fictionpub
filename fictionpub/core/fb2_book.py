...
class FB2Book:
    ...
    def get_quick_metadata(filepath: Path) -> QuickMetadata:
        ...
        try:
            ...
            if str(filepath).lower().endswith(".fb2.zip"):
                ...
                fb2_name = next(
                    (n for n in opened_zip.namelist() if n.lower().endswith(".fb2")),
                    None,
                )
                if not fb2_name:
            ...
            with zipfile.ZipFile(self.filepath, "r") as zf:
                ...
                if not fb2_files:
                    raise FileNotFoundError(
                        f"No .fb2 file found inside {self.filepath.name}."
                    )
    ...
    def _extract_metadata(self) -> None:
        ...
        """
        desc: etree._Element | None = xu.elem_find(self.tree, ".//fb:description")
        if desc is None:
        ...
        # --- title-info ---
        title_info_el: etree._Element | None = xu.elem_find(desc, "fb:title-info")
        if title_info_el is not None:
            meta.title_info = FB2Book._parse_title_info(title_info_el)

        # --- src-title-info ---
        src_el: etree._Element | None = xu.elem_find(desc, "fb:src-title-info")
        if src_el is not None:
            meta.src = FB2Book._parse_src_info(src_el)

        # --- document-info ---
        doc_el: etree._Element | None = xu.elem_find(desc, "fb:document-info")
        if doc_el is not None:
            meta.doc = FB2Book._parse_doc_info(doc_el)

        # --- publish-info ---
        pub_el: etree._Element | None = xu.elem_find(desc, "fb:publish-info")
    ...
    def _parse_title_info(el: etree._Element) -> TitleInfo:
        ...
        ti.authors = [xu.get_person_name(a) for a in xu.elem_findall(el, "fb:author")]
        ti.translators = [
            xu.get_person_name(t) for t in xu.elem_findall(el, "fb:translator")
        ]
        ti.annotation_el = xu.elem_find(el, "fb:annotation")
        ti.genres = [g.text for g in xu.elem_findall(el, "fb:genre") if g.text]
    ...
    def _parse_doc_info(el: etree._Element) -> DocumentInfo:
        ...
        tags = xu.get_metadata_tags(
            el, ["program-used", "date", "id", "version", "src-ocr"]
        )
        return DocumentInfo(
    ...
    def _extract_binaries(self) -> None:
