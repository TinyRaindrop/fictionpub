...
def int_tuple():
    ...
    def checker(value) -> tuple[int, int]:
        tvalue = tuple(int(v.strip()) for v in value.split(","))
        if len(tvalue) != 2:
            raise argparse.ArgumentTypeError(
                f"Value must be a comma separated (int, int) tuple, got {tvalue}"
            )
        return tvalue
...
def run_cli() -> None:
    ...
    parser.add_argument(
        "-c", "--css", type=Path, default=None, help="Path to a custom CSS file."
    )
    parser.add_argument(
        "-typ",
        "--typography",
        action="store_true",
        help="Enable typography post-processing.",
    )
    parser.add_argument(
        "-typ-nbsp",
        type=int_tuple(),
        default=(1, 1),
        help="Typography: word length range to add NBSP (int, int).",
    )
    parser.add_argument(
        "-typ-nobr",
        type=int_tuple(),
        default=(4, 6),
        help="Typography: word length range to wrap in <span>.nobreak (int, int).",
    )
    parser.add_argument(
        "--threads",
        type=int,
    ...
    def _progress_callback(result: ConversionResult) -> None:
        nonlocal completed_count, success_count, warning_count, failure_count
        completed_count += 1
        ...
        match result.status:
            case ConversionStatus.SUCCESS:
                success_count += 1
                print(f"{prefix} ✅ Done: {result.path.name}", flush=True)

            case ConversionStatus.WARNING:
                warning_count += 1
                print(f"{prefix} ⚠️  Done: {result.path.name}", flush=True)

            case ConversionStatus.FAILURE:
                failure_count += 1
                print(f"{prefix} ❌ Fail: {result.path.name}", flush=True)
                ...
                log.error(
                    f"Failed to convert {result.path.name}: {result.error}",
                    exc_info=False,
                )

    processor.run(files_to_process, _progress_callback)

    print("\nBatch conversion finished.")
    if warning_count > 0:
        print(
            "Some files had warnings. Most likely, minor issues with FB2. Check logs for details."
        )
    print(
